...
public class GetRecipientsRepositoryImpl implements GetRecipientsRepository {

  private final DynamoDbIndex<RecipientEntity> dynamoDbIndex;
  ...
  public PaginatedResult<Recipient> findByQuery(GetRecipientsQuery query) {
    ...
    try {
      ...
      List<Recipient> resultItems = items;
  ...
  private boolean isValidStartKey(
    ...
    try {
      ...
      var extractedBankAccountId =
          RecipientEntityKey.extractBankAccountId(pkAttr.s()).value();
  ...
  QueryConditional buildQueryConditional(GetRecipientsQuery query) {
    final var partitionKey = RecipientEntityKey.partitionKey(query.bankAccountId());
    final var queryParams = query.queryParams();
  ...
  private void recordItemsReturned(int count) {
    DistributionSummary.builder("account_recipients_repository_items_returned")
        .description("Number of Recipients returned per query")
