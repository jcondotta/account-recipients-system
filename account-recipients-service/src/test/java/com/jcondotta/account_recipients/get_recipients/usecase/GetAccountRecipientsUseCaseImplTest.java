...
class GetAccountRecipientsUseCaseImplTest {
  ...
  //    @BeforeEach
  //    void setUp() {
  //        useCase = new GetAccountRecipientsUseCaseImpl(queryMapperMock, cacheStoreMock,
  // repositoryMock);
  ...
  //        assertThat(result)
  //            .satisfies(it -> {
  //                assertThat(it.accountRecipients()).containsExactly(recipientDetails1,
  // recipientDetails2);
  ...
  //        var cachedAccountRecipientsResult =
  // GetAccountRecipientsResult.of(cachedRecipientDetailsList, null);
  //
  //
  ...
  //        when(repositoryMock.findByQuery(queryMock)).thenThrow(new RuntimeException("database
  // error"));
  //
  //        try {
