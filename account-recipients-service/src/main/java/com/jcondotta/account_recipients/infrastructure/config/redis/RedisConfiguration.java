...
public class RedisConfiguration {
  ...
  public CacheStore<GetAccountRecipientsResult> getAccountRecipientsResultCacheStore(
    ...
    return new RedisCacheStore<>(
        template, Duration.ofHours(24), GetAccountRecipientsResult.class, metricsRecorder);
  }

  /**
   * Factory method to build a type-safe RedisTemplate using a Jackson2JsonRedisSerializer for the
   * given type.
   */
  private <T> RedisTemplate<String, T> createTypedRedisTemplate(
      RedisConnectionFactory connectionFactory, ObjectMapper objectMapper, Class<T> valueClass) {
    RedisTemplate<String, T> template = new RedisTemplate<>();
    template.setConnectionFactory(connectionFactory);

    var keySerializer = new StringRedisSerializer();
    var valueSerializer = new Jackson2JsonRedisSerializer<>(objectMapper, valueClass);
