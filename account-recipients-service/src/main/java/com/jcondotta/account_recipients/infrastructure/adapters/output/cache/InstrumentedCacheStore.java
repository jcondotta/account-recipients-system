 ...
 * errors e latência.
 *
 * <p>Exemplo de uso: public class RedisCacheStore extends InstrumentedCacheStore<String,
 * AccountRecipients> { protected Optional<AccountRecipients> doGet(String key) { ... } protected
 * void doPut(String key, AccountRecipients value) { ... } protected void doEvict(String key) { ...
 * } }
 */
@Slf4j
public abstract class InstrumentedCacheStore<K, V> {
  ...
  protected InstrumentedCacheStore(MeterRegistry meterRegistry, String cacheName) {
    ...
    this.misses =
        Counter.builder("cache_misses_total").tag("cache", cacheName).register(meterRegistry);
    this.puts = Counter.builder("cache_puts_total").tag("cache", cacheName).register(meterRegistry);
    this.evicts =
        Counter.builder("cache_evictions_total").tag("cache", cacheName).register(meterRegistry);
    this.errors =
        Counter.builder("cache_errors_total").tag("cache", cacheName).register(meterRegistry);
    this.loadTimer =
